...
from src.logic.components.cluster_relation_component import (
    ClusterRelationshipClassificationComponent,
)
...
def test_cluster_relation_component():
    prepare()
    ...
    result = ClusterRelationshipClassificationComponent("Machine Learning").run(
        clusters
    )
...
def test_relation_component():
    ...
    result = RelationshipClassificationComponent("Machine Learning", 2).run(documents)
    assert len(result["relations"]) == 1
    assert result["relations"][0].document_A in [documents[0].id, documents[1].id]
    assert result["relations"][0].document_B in [documents[0].id, documents[1].id]
    assert result["relations"][0].document_A != result["relations"][0].document_B
